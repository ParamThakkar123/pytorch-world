...
def load_single_log_db_data(
    ...
    for idx, img_item in enumerate(images_data):
        ...
        img_path = (
            f"{log_db._data_root}/nuplan-v1.1/sensor_blobs/{img_item.filename_jpg}"
        )
        ...
        if (scene_token != pre_scene_token) and (pre_scene_token is not None):
            print(
                f"idx: {idx}, scene_token:{scene_token}, next_token:{next_img_token}, {camera_channel}, img_path:{img_item.filename_jpg}"
            )
...
if __name__ == "__main__":
    ...
    split_name = "train"
    ego_save_dir = f"{ROOT}/nuplan-v1.1/ego_meta/{split_name}"
    seq_save_dir = f"{ROOT}/nuplan-v1.1/seq_meta/{split_name}"
    ...
    if split_name == "train":
        split_db_list_update = [
            db_i
            for db_i in split_db_list
            if os.path.exists(f"{ROOT}/nuplan-v1.1/splits/trainval/{db_i}.db")
        ]
        db_path_lists = [
            f"{ROOT}/nuplan-v1.1/splits/trainval/{db_i}.db"
        ...
        split_db_list_update = [
            db_i
            for db_i in split_db_list
            if os.path.exists(f"{ROOT}/nuplan-v1.1/splits/test/{db_i}.db")
        ]
        db_path_lists = [
            f"{ROOT}/nuplan-v1.1/splits/test/{db_i}.db" for db_i in split_db_list_update
        ]
    ...
    print("Start loop.")
    accumulated_results = loop_over_db_files(
        0,
        1,
        split_db_list_update,
        ego_save_dir=ego_save_dir,
